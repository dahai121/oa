	function video_51cto($field, $value, $fieldinfo) {
		extract($fieldinfo);
		$list_str = '';
		if($value) {
		...
		$string .= $str."<div class='picBut cu'><a herf='javascript:void(0);' onclick=\"javascript:flashupload('{$field}_images', '".L('attachment_upload')."','{$field}',change_images,'{$upload_number},{$upload_allowext},{$isselectimage}','content','$this->catid','{$authkey}')\"/> ".L('multiple_file_list')." </a></div>";
		return $string;
	}