...
class sitemodel extends admin {
	...
	public function init() {
		...
		$datas = $this->db->listinfo(array('siteid'=>$this->siteid,'type'=>3),'',$_GET['page'],8);
		//模型文章数array('模型id'=>数量);
		$items = array();
		...
		$this->public_cache();
        //更新缓存
        pc_base::load_app_class('people_cache','','');
        people_cache::update_cache_model();
		$big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=people&c=sitemodel&a=add\', title:\''.L('add_model').'\', width:\'580\', height:\'420\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('add_model'));
	...
	public function add() {
		if(isset($_POST['dosubmit'])) {
            ...
            $model_sql = file_get_contents(MODEL_PATH.'people_model.sql');
			...
			$admin_list_template = $this->admin_people_list_template('people_list', 'name="setting[admin_list_template]"');
	...
	public function edit() {
		if(isset($_POST['dosubmit'])) {
			...
			//var_dump($_POST['info']['admin_list_template']);die;
			...
			extract($r);
			$admin_list_template_f = $this->admin_people_list_template($admin_list_template, 'name="setting[admin_list_template]"');
	...
	public function public_cache() {
		...
		//更新模型数据缓存
		$model_array = array();
		$datas = $this->db->select(array('type'=>3));
		foreach ($datas as $r) {
