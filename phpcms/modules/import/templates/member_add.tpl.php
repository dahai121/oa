...
<fieldset>
    ...
    <td> 
    <?php echo form::radio(array('gbk'=>'GBK', 'gb2312'=>'gb2312', 'utf8'=>'UTF8','latin1'=>'latin1'),$setting['dbcharset'],'name="setting[dbcharset]" id="dbcharset"',L('please_select'));?>
	</td>
	</tr>
...
<fieldset>
	...
	<table width="100%" class="table_form">
     ...
     <tr>
		...
		<input type="radio" name="setting[membercheck]" value="1" <?php if($setting['membercheck']==1){echo 'checked';}?> class="radio_style"> <?php echo L('yes')?>&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="setting[membercheck]" value="0" class="radio_style" <?php if($setting['membercheck']=='0'){echo 'checked';}?>> <?php echo L('no')?> 
		<?php }else{?>
		<input type="radio" name="setting[membercheck]" value="1" checked class="radio_style"> <?php echo L('yes')?>&nbsp;&nbsp;&nbsp;&nbsp;<input type="radio" name="setting[membercheck]" value="0" class="radio_style"> <?php echo L('no')?> 
		<?php }?>
...
$('#testdb').click(function(){
	...
	if(data=='OK') 
	{
		alert('<?php echo L('connect_succeed')?>');
	}
	else
	{
		alert('<?php echo L('connect_fail')?>');
	}
	});
});
