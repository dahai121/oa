...
<div class="pad-10">
    <div class="common-form">
        <form name="myform" action="?m=ren&c=health&a=add" method="post" id="myform"  enctype="application/x-www-form-urlencoded">
            <fieldset>
                ...
                <table width="100%" class="table_form">
                    ...
                    <tr>
                        ...
                        <td><textarea name="info[jzyy]" ></textarea></td>
                    </tr>
                    <tr>
                        <td width="80"><?php echo L('主要病症') ?></td>
                        <td><textarea name="info[zybz]" ></textarea></td>
                    </tr>
...
<script>
    $(function () {
        if($(".rylx").val() == "") {
            $.post("?m=ren&c=zhuyuan&a=ajax_get_people&pc_hash=<?php echo $_SESSION['pc_hash'];?>", {id: ''}, function (data) {
                $("#name").html(data);
            });
        }

        $(".rylx").change(function(){console.log($(".rylx").val());
           var id =  $(this).val();
            $.post("?m=ren&c=zhuyuan&a=ajax_get_people&pc_hash=<?php echo $_SESSION['pc_hash'];?>",{id:id},function(data){
                $("#name").html(data);
            });
        });
        ...
        $("#name").change(function(){
            var id =  $("#name select").val();
            $.post("?m=ren&c=zhuyuan&a=ajax_get_dy&pc_hash=<?php echo $_SESSION['pc_hash'];?>",{id:id},function(data){
                var aaa = eval('('+data+')');
                $("#ylxh").html(aaa.ybxh);
