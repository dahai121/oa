...
class health extends admin {
    ...
    function __construct() {
        parent::__construct();
            $this->db = pc_base::load_model('health_model');
...
//        var_dump( $memberlist_arr);
        ...
        $memberlist_arr = $this->db->listinfo($where, 'id DESC', $page, 15);
        $pages = $this->db->pages;
        $big_menu = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=ren&c=health&a=add&modelid='.$modelid.'\', title:\''.L('添加人员').'\', width:\'1200\', height:\'500\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', L('添加人员'));
        include $this->admin_tpl('health_list');
    }
    ...
    function add() {
        ...
        if(isset($_POST['dosubmit'])) {
            ...
            include $this->admin_tpl('health_add');
        }
