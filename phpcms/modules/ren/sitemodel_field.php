...
class sitemodel_field extends admin {
	...
	public function init() {
        ...
        //更新缓存
		$a = $this->cache_field($modelid);
		$datas = $this->db->select(array('modelid'=>$modelid),'*',100,'listorder ASC');

        $r = $this->model_db->get_one(array('modelid'=>$modelid));

        require MODEL_PATH.'peoplefields.inc.php';
        include $this->admin_tpl('peoplemodel_field_manage');
    }
	//添加字段
	public function add() {
		if(isset($_POST['dosubmit'])) {

			$model_cache = getcache('people_model','commons');
			...
			showmessage(L('add_success'),'?m=people&c=sitemodel_field&a=init&modelid='.$modelid.'&menuid=59');
		} else {
			...
			include $this->admin_tpl('peoplemodel_field_add');
		}
	}
	//修改字段
	public function edit() {
		if(isset($_POST['dosubmit'])) {
            ...
            require MODEL_PATH.'peopleedit.sql.php';
            //附加属性值
			$_POST['info']['setting'] = array2string($_POST['setting']);
			...
			$this->cache_field($modelid);
			showmessage(L('update_success'),'?m=people&c=sitemodel_field&a=init&modelid='.$modelid.'&menuid=59');
		} else {
            ...
            //获取下拉列表
            $xia_info = $this->xiala->select(array('pid'=>'0'));
			...
			header("Cache-control: private");
			include $this->admin_tpl('peoplemodel_field_edit');
		}
	}
	...
	public function delete() {
		...
		$modelid = intval($_GET['modelid']);
		$model_table = $model_cache[$modelid]['tablename'];
		$tablename = $r['issystem'] ? $model_table : $model_table;
	...
	public function get_select(){
        $info = $this->xiala->select(array('pid'=>'0'));
        foreach ($info as $k =>$v){
            //数据转换为utf8格式（json支持utf8）
            $a[]=array_iconv($v);
        }
        echo json_encode($a);
    }
