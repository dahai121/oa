...
class content extends foreground {
	...
	public function publicfolders() {
  	$catid=42;
  	if(!isset($catid)) showmessage(L('illegal_parameters'));
    ...
    //$_dept=get_memberdetail($this->_userid,'dept');
			...
			if(is_array($_POST['messageid'])){
					...
					//删除内容
					$content_db->delete_content($messageid_arr); //删除文章
					//删除统计表数据
					$this->hits_db->delete(array('hitsid'=>'c-'.$modelid.'-'.$messageid_arr));
					//删除附件
					$attachment->api_delete('c-'.$catid.'-'.$messageid_arr);
					//删除审核表数据
					$this->content_check_db->delete(array('checkid'=>'c-'.$messageid_arr.'-'.$modelid));
					//删除推荐位数据
					$this->position_data_db->delete(array('id'=>$messageid_arr,'catid'=>$catid,'module'=>'content'));
					//删除全站搜索中数据
					$this->search_db->delete(array('id'=>$messageid_arr));
					}
				showmessage(L('operation_success'), HTTP_REFERER);
      } else {
				$keyword = trim($_GET['keyword']);
				$sql = "select e.*,d.* from ".$table_name." as e,".$table_name."_data as d where e.id = d.id and e.status=99 ";
				//if($keyword){
				...
				include template('member', 'content_gxwjj_list');
			}
  }
  ...
  public function officedocuments() {
    ...
    $_dept=get_memberdetail($this->_userid,'dept');
			...
			if(is_array($_POST['messageid'])){
					...
					$this->position_data_db->delete(array('id'=>$messageid_arr,'catid'=>$catid,'module'=>'content'));
					//删除全站搜索中数据
					$this->search_db->delete(array('typeid'=>$typeid,'id'=>$messageid_arr));
					}
				showmessage(L('operation_success'), HTTP_REFERER);
      } else {
				$keyword = trim($_GET['keyword']);
				$sql = "select e.*,d.* from ".$table_name." as e,".$table_name."_data as d where e.id = d.id and e.status=99 and e.dept=$_dept ";
				//if($keyword){
				//    $where = "and (e.title like '%$keyword%' or d.expcon like '%$keyword%') ";
				//}
				//$sql= $sql.$where." order by e.id desc"; 
				...
				$infos = $search->multi_listinfo($sql,$page); //返回查询结果
				$pages = $search->pages;//返回分页
				include template('member', 'content_cswjj_list');
      }
  }
