...
class admin {
     ...
     * @param string $str form表单中的属性名
     */
    final public function admin_people_list_template($id = '', $str = '') {
        $templatedir = PC_PATH.DIRECTORY_SEPARATOR.'modules'.DIRECTORY_SEPARATOR.'people'.DIRECTORY_SEPARATOR.'templates'.DIRECTORY_SEPARATOR;
        $pre = 'people_list';
        $templates = glob($templatedir.$pre.'*.tpl.php');
        if(empty($templates)) return false;
        $files = @array_map('basename', $templates);
